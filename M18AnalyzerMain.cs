...
// serial protocol implementation in M18Protocol.cs as well as the FTDI discovery helper
// FtdiDeviceUtil.cs. Every method and key line is commented to teach WinForms fundamentals (event
...
using System.Collections.Generic; // Supplies List<T> for storing FTDI device objects.
...
{
    /// <summary>
    /// The primary WinForms window that presents user controls for FTDI device selection,
    ...
    /// <see cref="FtdiDeviceUtil"/> to enumerate FTDI devices with extended metadata.
    /// </summary>
    public partial class frmMain : Form
    {
        ...
        // Track whether the raw D2XX log RichTextBox already has content.
        private bool _hasAppendedD2xxLog; // Prevents inserting extra blank lines before first raw D2XX entry.
        // Store the currently selected FTDI device for connection attempts.
        private FtdiDeviceDisplay? _selectedDevice; // Persisted when user picks an item from cmbBxSerialPort.
        // Store a human-friendly device description for user messages.
        private string? _selectedDeviceDescription; // Derived from FtdiDeviceDisplay to show manufacturer/USB metadata.
        ...
        public frmMain()
        {
            ...
            btnRefresh.Click += btnRefresh_Click; // Refresh list of FTDI devices when "Refresh" pressed.
            btnConnect.Click += btnConnect_Click; // Attempt to open FTDI device when "Connect" pressed.
            btnDisconnect.Click += btnDisconnect_Click; // Close FTDI device when "Disconnect" pressed.
            ...
            cmbBxSerialPort.SelectedIndexChanged += cmbBxSerialPort_SelectedIndexChanged; // Capture selected FTDI device.
            FormClosing += frmMain_FormClosing; // Ensure we cleanly close FTDI device when form closes.
            ...
            toolTipSimpleTab.SetToolTip(btnRefresh, "Refresh the list of available FTDI devices."); // Helps beginners understand UI flow.
            toolTipSimpleTab.SetToolTip(btnConnect, "Connect to the selected FTDI device."); // Explains connection button.
        ...
        /// <summary>
        /// Executes when the form finishes loading. We immediately refresh FTDI devices so the
        /// combo box is populated before the user interacts with it.
        /// </summary>
        private void frmMain_Load(object sender, EventArgs e)
        {
            RefreshSerialPorts(); // Kick off device enumeration via FtdiDeviceUtil.
        }
        ...
        private void btnRefresh_Click(object? sender, EventArgs e)
        {
            ...
            RefreshSerialPorts(); // Pull latest FTDI devices using FtdiDeviceUtil.
        }

        /// <summary>
        /// Enumerates available FTDI devices, populates the combo box, and logs rich metadata for
        /// troubleshooting USB cables/adapters. Uses <see cref="FtdiDeviceUtil.EnumerateDevices"/>.
        /// </summary>
        private void RefreshSerialPorts()
        {
            AppendLogBoth("Refreshing FTDI device list..."); // Inform user in both simple and debug logs.

            try
            {
                var devices = GetFtdiDevices(); // Gather FTDI info via D2XX (plus SetupAPI for COM mapping).

                cmbBxSerialPort.Items.Clear(); // Reset combo box so old entries do not linger.
                foreach (var device in devices)
                {
                    cmbBxSerialPort.Items.Add(device); // Add FTDI record; ToString() shows DisplayName.
                    AppendLogBoth($"Found FTDI device {device.DisplayName}"); // Echo discovery to the log.
                }

                AppendDebugLog($"RefreshSerialPorts(): {devices.Count} FTDI device(s) detected."); // Debug summary count.
                foreach (var device in devices)
                {
                    ...
                    AppendDebugLog($"   Details: description='{device.Description}', serial='{device.SerialNumber}', COM='{device.ComPort ?? "(none)"}', manufacturer='{device.Manufacturer ?? "(none)"}'"); // Show manufacturer/hardware IDs for driver troubleshooting.
                }

                if (devices.Count == 0)
                {
                    AppendLogBoth("No FTDI devices detected."); // Warn user if no FTDI cables are found (e.g., driver missing).
                }
                else
                {
            ...
            {
                LogError("Error while refreshing FTDI devices", ex); // Surface exceptions with MessageBox to aid debugging.
            }
        }

        /// <summary>
        /// Captures combo-box selection changes so we know which FTDI device to open.
        /// </summary>
        private void cmbBxSerialPort_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (cmbBxSerialPort.SelectedItem is FtdiDeviceDisplay selected)
            {
                _selectedDevice = selected; // Save FTDI device entry for connection.
                _selectedDeviceDescription = selected.DisplayName; // Save descriptive text for user prompts.
                AppendDebugLog($"Selected FTDI device set to {selected.DisplayName} (source: {selected.Source})."); // Log the selection for trace.
            }
            else
            {
        ...
        private async void btnConnect_Click(object? sender, EventArgs e)
        {
            ...
            if (cmbBxSerialPort.SelectedItem is not FtdiDeviceDisplay selectedPort)
            {
                AppendLog("No FTDI device selected. Please choose a device before connecting."); // User guidance when no selection exists.
                MessageBox.Show("Please select an FTDI device before connecting.", "FTDI Device", MessageBoxButtons.OK, MessageBoxIcon.Warning); // Modal dialog prevents accidental action.
                return; // Abort connect flow.
            }

            if (_protocol != null)
            {
                if (string.Equals(_protocol.port.Device.SerialNumber, selectedPort.SerialNumber, StringComparison.OrdinalIgnoreCase))
                {
                    AppendLogBoth($"Device {selectedPort.DisplayName} is already open. Ignoring duplicate connect request."); // Avoid reopening same port.
                    MessageBox.Show($"{selectedPort.DisplayName} is already open.", "FTDI Device", MessageBoxButtons.OK, MessageBoxIcon.Information); // Notify user.
                    return; // Exit without reopening.
                }

                AppendLogBoth($"A different device ({_protocol.port.Device.SerialNumber}) is currently open. Closing it before opening {selectedPort.DisplayName}..."); // Explain why we close first.
            ...
            AppendLogBoth($"Attempting to open {selectedDescription} with D2XX settings: 4800 baud, 8 data bits, parity None, stop bits Two."); // Describe UART config so readers learn standard serial settings.
            AppendDebugLog("D2XX connection will set TX low (idle) after open via FT_SetBreakOn + FT_SetDtr."); // Note initial electrical state.

            try
            {
                await Task.Run(() => _protocol = new M18Protocol(selectedPort, AppendD2xxLog)); // Instantiate protocol off-UI thread; constructor opens FTDI handle and drives TX idle.
        ...
        private async void btnDisconnect_Click(object? sender, EventArgs e)
        {
            ...
            await DisconnectAsync(); // Attempt to close the FTDI device and dispose protocol safely.
        }

        /// <summary>
        /// Closes the active protocol/FTDI device if present, optionally informing the user via
        ...
        private async Task DisconnectAsync(bool showUserMessages = true)
        {
            if (_protocol == null)
            {
                var message = "Disconnect requested, but no FTDI device is currently open."; // Friendly explanation.
                ...
                if (showUserMessages)
                {
                    ...
                    MessageBox.Show(message, "FTDI Device", MessageBoxButtons.OK, MessageBoxIcon.Information); // Notify via modal dialog.
                }
                UpdateConnectionUi(false); // Reset UI state regardless.
                return; // Exit early when nothing to close.
            }

            AppendLogBoth($"Closing {_protocol.port.Device.DisplayName}..."); // Inform user which device is closing.
            ...
            try
            {
                await Task.Run(() =>
                {
                    if (_protocol.port.IsOpen)
                    {
                        ...
                        _protocol.port.Close(); // Close the underlying FTDI handle.
                    }
                }); // Execute Close on background thread; sets TX idle then disposes FTDI handle.
        ...
        private async void btnIdle_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                AppendLogBoth("TX set to Idle (low). Safe to connect or disconnect battery. (See Raw D2XX Log for FT_SetBreakOn/FT_SetDtr calls.)"); // Teach user about safe state.
            }
            catch (Exception ex)
            {
        ...
        private async void btnActive_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                AppendLogBoth("TX set to Active (high). Charger simulation enabled. (See Raw D2XX Log for FT_SetBreakOff/FT_ClrDtr calls.)"); // Show user the state change.
            }
            catch (Exception ex)
            {
        ...
        private bool EnsureConnected()
        {
            ...
            AppendLog("No active connection. Please connect to an FTDI device first."); // User-friendly guidance.
            MessageBox.Show("Please connect to an FTDI device first.", "Connection required", MessageBoxButtons.OK, MessageBoxIcon.Warning); // Modal warning clarifies next step.
            return false; // Signal caller to abort operation.
        }

        /// <summary>
        /// Wrapper around FtdiDeviceUtil to retrieve detailed device metadata, keeping the logic
        /// isolated for easier unit testing and mocking.
        /// </summary>
        private List<FtdiDeviceDisplay> GetFtdiDevices()
        {
            return FtdiDeviceUtil.EnumerateDevices(AppendD2xxLog); // Pass raw logger to capture D2XX enumeration details.
        }
        ...
        private async void btnTestFT232_Click(object? sender, EventArgs e)
        {
            ...
            if (cmbBxSerialPort.SelectedItem is not FtdiDeviceDisplay selectedPort)
            {
                AppendLog("No FTDI device selected. Please choose a device before testing."); // Provide guidance.
                MessageBox.Show("Please select an FTDI device before testing.", "FTDI Device", MessageBoxButtons.OK, MessageBoxIcon.Warning); // Block until user acknowledges.
                return; // Abort test.
            }

            var selectedDescription = _selectedDeviceDescription ?? selectedPort.DisplayName; // Friendly name for messages.

            AppendLog($"Testing FT232 on {selectedDescription} via D2XX..."); // Inform user test is starting.
            LogDebugAction($"Testing FT232 on {selectedDescription} via D2XX."); // Duplicate in debug log.
        ...
        /// <summary>
        /// Opens the specified device with the FTDI-friendly serial settings and performs a minimal
        /// control-line toggle plus a single-byte write to check for driver/hardware responsiveness.
        /// </summary>
        private static (bool Success, string? ErrorMessage) TestSerialDevice(FtdiDeviceDisplay device)
        {
            try
            {
                using var d2xxPort = new FtdiD2xxPort(device, null); // Open FTDI handle with default logger.
                d2xxPort.SetDtr(true); // Assert DTR to mimic idle charger line initially.
                d2xxPort.SetBreak(true); // Assert BREAK to drive TX low.
                d2xxPort.SetDtr(false); // Toggle DTR to confirm control-line responsiveness.
                d2xxPort.SetBreak(false); // Release BREAK to drive TX high.

                d2xxPort.WriteBytes(new byte[] { 0x00 }); // Send a single zero byte out TX to verify TX data path.
        ...
        private void LogError(string context, Exception exception)
        {
            ...
            MessageBox.Show(fullMessage, "FTDI Device Error", MessageBoxButtons.OK, MessageBoxIcon.Error); // Display blocking error dialog.
        }
        ...
        private void ApplyProtocolLoggingPreferences()
        {
            ...
            _protocol.RawLogger = AppendD2xxLog; // Route raw FTDI call traces into the dedicated D2XX tab.
        }
        ...
        /// <summary>
        /// Writes text into the raw D2XX log rich text box, mirroring FTDI API calls for visibility.
        /// Background threads are marshaled to the UI thread to avoid cross-thread exceptions.
        /// </summary>
        private void AppendD2xxLog(string message)
        {
            if (rtbD2xxLog.InvokeRequired)
            {
                rtbD2xxLog.Invoke(new Action(() => AppendD2xxLog(message))); // Marshal to UI thread if needed.
                return; // Stop processing on worker thread.
            }

            var prefix = _hasAppendedD2xxLog ? Environment.NewLine : string.Empty; // Add newline after first message.
            var formatted = message.Contains("- ") ? message : FormatLogMessage(message); // Preserve existing timestamps if provided.
            rtbD2xxLog.AppendText($"{prefix}{formatted}"); // Write to raw D2XX output box.
            rtbD2xxLog.SelectionStart = rtbD2xxLog.TextLength; // Move caret to end.
            rtbD2xxLog.ScrollToCaret(); // Auto-scroll to newest entry.
            _hasAppendedD2xxLog = true; // Note content exists.
        }
        ...
        /// the open FTDI device. This is similar to a ping test for UART communications.
        /// </summary>
        private void btnTestEcho_Click(object sender, EventArgs e)
        {
            rtbDebugOutput.AppendText($"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} - Starting raw echo test on {_protocol?.port.Device.SerialNumber ?? "??"}\n"); // Timestamp test start with chosen device.
            try
            {
                if (_protocol?.port.IsOpen != true)
                {
                    rtbDebugOutput.AppendText($"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} - FTDI device not open. Connect first.\n"); // Warn user that port must be open.
                    return; // Exit early because no active FTDI handle exists.
                }
                ...
                _protocol.port.WriteBytes(send); // Write to UART TX line via D2XX; FT232 shifts bits out over USB-UART bridge.
